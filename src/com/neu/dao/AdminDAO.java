...
public class AdminDAO extends HibernateDaoSupport  {
    ...
    public Admin findById( java.math.BigDecimal id) {
        log.debug("getting Admin instance with id: " + id);
        try {
            Admin instance = (Admin) getHibernateTemplate()
                    .get("com.neu.dao.Admin", id);
            return instance;
        } catch (RuntimeException re) {
	...
	public boolean login(String username, String userpass){
		...
		boolean b = false;
		String hql = "from Admin where username=? and userpass=?";
		List list = getHibernateTemplate().find(hql,username,userpass); 
		if(list.size() > 0){
			b = true;
		}
		return b;
